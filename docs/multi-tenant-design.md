...
**Status:** Accepted — Option D (§3). Phase 0 (`beb6f85`); 1a (`dc05015`); 1b (`997c62c`); 1c (`f035f27`); 1d shipping (this commit). Lifting the 503 guard waits on Phase 2 (per-tenant Neo4j) — Postgres-side isolation is enough on its own only after Neo4j catches up.
...
2. **Phase 1 — Postgres schema-per-tenant cutover.** Sub-phased to keep individual changes reviewable:
   ...
   - **1d — audit refactor + simple table moves.** `audit_log` gains a `tenant_id NOT NULL` column with FK to `control.tenants`; the table stays in `public` because the partitioned layout makes a SET SCHEMA move expensive and cross-tenant audit reads are a feature for super-admin. `audit-buffer.js` now requires `tenantId` on every row. `fastify.pg.auditFor(tenantId)` returns a curried audit function; `tenantContext` attaches `req.audit` to every authenticated request, and 15+ call sites switched from `fastify.pg.audit(...)` / a local `audit(...)` helper to `req.audit(...).catch(() => {})`. `audit.js` routes filter by `tenant_id` when `req.tenant` is set (cross-tenant reads remain available to super-admin without `X-Tenant-Slug`). Three FK-free tables — `terraform_imports`, `ai_jobs`, `discovery_schedule` — moved to `tenant_default` (templates 003-005, cutover migration 18). Scheduler now reads its schedule via `forTenant('tenant_default')`. **— Shipped.**
   - **1e — remaining tables + lift the 503 guard.** Move `cmdb_assessment_*` (multi-table, complex) and `otel_tenants` / `otel_spans_raw` (column-name collision on `tenant_id`, partition layout). Per-tenant `discovery_schedule` iteration in the scheduler so each tenant can pick its own interval. The 503 guard's lift then waits on Phase 2 (per-tenant Neo4j) — Postgres-side isolation is necessary but not sufficient.
